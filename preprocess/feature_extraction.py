"""
    ...
    extract features from pcaps by DPDK
...
def extract_features():
    ...
    """
    DPDK Environment
        ssh dpdkgroup@10.100.1.204 (sender: MoonGen)
         tshark -F pcap -r examples/pcap/attackf3.pcap -w examples/pcap/attackf33.pcap
         sudo ./build/MoonGen examples/pcap/replay-pcap.lua 1  examples/pcap/attackf33.pcap

        ssh dpdkgroup@10.100.3.1.   (receiver: our code (sess_stats) to extract features (27 features))
        sudo ./build/sess_stat - 0 -n 2 -- -L attackf22 -t-1 m 100000000
        (sudo ./build/sess_stat -l 0 -n 2 -- -P 1 -p 0x3 -L sess_log -t -1 -m 10000000)
...
def share_files_by_website():
    """
        share files with website: transfer.sh or file.io
        'curl --upload-file xxx.txt https://transfer.sh/xxx.txt'
