...
def train_test_traditional_algorithms(experiment=('key', '', 'unb', 'unb'), datasets_dict='', params_dict=''):
    """
    ...
    train_set_name = experiment[-1] + '_train_set'
    val_set_name = experiment[-1] + '_val_set'
...
def train_test_AE(experiment=('key', [], 'unb', 'unb'), datasets_dict='', params_dict=''):
    """
    ...
    x_val = val_set_dict[val_set_name]['x']
    ...
    y_pred_label_AE, y_pred_probs_AE, _ = test_AE(test_set=train_set_dict[train_set_name], experiment=experiment,
...
def conduct_experiment_without_feature_selection(sub_experiment={}, datasets_dict={}, params_dict={}):
    """
    ...
    print(f"-step 2. train and test AE without feature selection: {sub_experiment}")
    ...
    if params_dict['show_flg']:
        for key, value_dict in all_y_test_pred_proba_dict.items():
            ...
            plot_roc(value_dict, out_file=out_file, balance_data_flg=True, title_flg=params_dict['title_flg'],
                     title=title)
...
def conduct_experiment_2(sub_experiment={}, params_dict=''):
    """
    ...
    datasets_dict = get_dataset(sub_experiment['experiment_value'], input_dir=params_dict['input_dir'],
                                                      random_state=params_dict['random_state'])

    print("-step 1.1. preprocessing data")
    datasets_dict = normalize_all_data(datasets_dict=datasets_dict, train_set_name=sub_experiment[-1], params_dict=params_dict)
...
def main(params_dict=''):
    ...
    if key == 'all':
        ...
        conduct_experiment_2(sub_experiment=('SYNT', 'uSc2C1_z-score_20_14'), params_dict=params_dict)
    else:
        pass
