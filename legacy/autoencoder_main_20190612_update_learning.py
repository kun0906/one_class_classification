...
def train_AE(AE_model_file, train_data, val_data='', case='', feat_size='', Epochs=2, batch_size=32):
    """
    ...
    if AE_model_file != '':
        model.load_weights(AE_model_file)
    ...
    model_file = "models_dumping/AE_" + str(feat_size) + '_' + case + ".hdf5"
    model.save_weights("models_dumping/AE_" + str(feat_size) + '_' + case + ".hdf5")

    return model_file, training_results.history


# To train the autoencoder model
def update_model(AE_model_out_file, DB_norm_data, train_data, val_data, case, feat_size, Epochs=2, batch_size=32):
    """
        only use normal samples to train AE.
    :param train_data:
    :param val_data:
    ...
    model.load_weights(AE_model_out_file)  # load parameters from last model.

    st = time.time()
    training_results = model.fit(DB_norm_data, DB_norm_data, epochs=Epochs,
                                 batch_size=batch_size,
                                 verbose=0)
    print('AE training for Case : ', case, ' = ', time.time() - st, 's')

    score = model.evaluate(DB_norm_data, DB_norm_data, batch_size=batch_size, verbose=0)
    print("Train Loss = ", score, 'model.metrics_names', model.metrics_names)

    # score = model.evaluate(val_data, val_data, batch_size=batch_size, verbose=0)
    # print("Validation Loss = ", score, 'model.metrics_names', model.metrics_names)

    model.save_weights(AE_model_out_file + 'new_model')

    new_AE_model = model

    return new_AE_model, training_results.history


# TO test the model
def online_detect_and_offline_training_updating_model(AE_model_file, DB_norm_data_lst, data_test, data_test_labels,
                                                      data_train, case, feat_size, thres_AE=1.5):
    """
    ...
    recall, fnr, fpr, tnr, acc = calucalate_metrics(data_test_labels, y_pred_label_AE)

    num_for_retrain = 1000
    for i, y_p in enumerate(y_pred_label_AE):
        if y_p == 1 and data_test_labels[i] == 1:  # normal
            DB_norm_data_lst.append(data_test[i])

    if len(DB_norm_data_lst) < num_for_retrain:
        print('there is no enough samples')
        return AE_model_file, DB_norm_data_lst, (recall, fnr, fpr, tnr, acc)
    y_norm_labels = np.ones((num_for_retrain, 1))
    train_set = np.asarray(DB_norm_data_lst, dtype=float)
    AE_model_file, _ = train_AE(AE_model_file, train_set, val_data=train_set, case=case, feat_size=feat_size, Epochs=20,
                                batch_size=32)
    DB_norm_data_lst = DB_norm_data_lst[num_for_retrain:]  # remove the first 1000 normal samples.
    ...
    # return AE_model_file, DB_norm_data_lst, y_pred_label_AE, y_pred_probs_AE, reconstr_errors_lst
    return AE_model_file, DB_norm_data_lst, (recall, fnr, fpr, tnr, acc)


def plot_online_results(online_res_dict, out_file='figures/online_detection_result.pdf', title_flg=True, title=''):
    fig, ax = plt.subplots()
    ax.plot(online_res_dict['fpr'], 'r*-', label='FPR')
    ax.plot(online_res_dict['fnr'], 'b*-', label='FNR')
    # ax.plot(num_features_list, res_metrics_lst['tpr'], 'g*-', label='TPR')
    # ax.plot(num_features_list, res_metrics_lst['tnr'], 'c*-', label='TNR')
    # plt.xlim([0.0, 1.0])
    # plt.ylim([0.0, 1.05])
    plt.xlabel('with time')
    plt.ylabel('Rate')
...
def random_select_test_data(x_test, y_test, random_state=42):
    # X, y = shuffle(x_test, y_test, random_state=random_state)
    #
    #
    # sample_num = 5000
    # # if len(y) < sample_num:
    # #     num = random.randint(len(y), sample_num)
    # # else:
    # #     num = random.randint(sample_num, len(y))
    # num = sample_num
    # print(f'random select {num} samples')
    #
    # return x_test[:num], y_test[:num]

    x_train_tmp, x_test_tmp, y_train_tmp, y_test_tmp = train_test_split(x_test, y_test,
                                                                        test_size=0.2,
                                                                        shuffle=True)

    return x_test_tmp, y_test_tmp
...
def experiment_1_without_feature_selection(Epochs, case, optimal_thres_AE, find_optimal_thres_flg, random_state,
    ...
    AE_model_file, training_losses = train_AE(AE_model_file, x_norm_train, x_norm_val, case, num_features, Epochs)
    ...
    if type(x_attack_1) != type(None):
        attacks_num = int(x_norm_test.shape[0] * 0.1 / x_attack_1.shape[0])  # only use 10 % attack in test set
        x_test = np.concatenate([x_norm_test, x_attack_1[:attacks_num]])
        y_test = np.concatenate([y_norm_test, y_attack_1[:attacks_num]])
    else:
        x_test = x_norm_test
        y_test = y_norm_test
    print('online_detect_and_offline_training_updating_model on case:', case)
    DB_norm_data_lst = []
    online_res_dict = {'tpr': [], 'fnr': [], 'fpr': [], 'tnr': [], 'acc': []}
    for idx in range(30):
        x_test_i, y_test_i = random_select_test_data(x_test, y_test, random_state=random_state)
        print(f'idx={idx}')
        AE_model_file, DB_norm_data_lst, (tpr, fnr, fpr, tnr, acc) = online_detect_and_offline_training_updating_model(
            AE_model_file,
            DB_norm_data_lst, x_test_i,
            y_test_i, '', case,
            num_features,
            thres_AE=optimal_thres_AE)
        online_res_dict['tpr'].append(tpr)
        online_res_dict['fnr'].append(fnr)
        online_res_dict['fpr'].append(fpr)
        online_res_dict['tnr'].append(tnr)
        online_res_dict['acc'].append(acc)

    out_file = f'figures/{case}_online_detection_result_with_{x_test.shape[1]}_features.pdf'
    plot_online_results(online_res_dict, out_file, title_flg=True, title='')
    ...
    # y_pred_label_AE, y_pred_probs_AE, reconstr_errs_arr = test_AE(x_test, y_test, x_norm_train, case, num_features, thres_AE=optimal_thres_AE)
    ...
    #         plot_reconstruction_errors_from_txt(input_file=out_file, balance_data_flg=True, random_state=random_state,title_flg=title_flg, title=title)
    #
    ...
    #     y_pred_label_AE, y_pred_probs_AE, _ = test_AE(x_test, y_test, x_norm_train, case, x_test.shape[1], thres_AE=thres)
    ...
    #
    # return all_res_metrics_feat_list, optimal_thres_AE, DB_txt
...
def main(Epochs=2, optimal_thres_AE=5, find_optimal_thres_flg=False, cases=['uSc3C2_min_max_20_14'], factor=10,
         ...
         balance_train_data_flg=True, title_flg=True, sub_features_flg=False, sub_features_lst=[], DB_txt=''):
    """
    ...
    for case in cases:
        if case[3] == '1':
            ...
            (x_norm_train_raw, y_norm_train), (x_norm_val_raw, y_norm_val), (x_norm_test_raw, y_norm_test), (
                x_attack_1_raw, y_attack_1) = load_data(case, random_state=random_state, test_size=0.8)
            ...
            if not analyize_features_flg:
                ...
                # sub_features_lst=[]
                AE_model_file = ''
                experiment_1_without_feature_selection(
                    Epochs, case,
                    optimal_thres_AE, find_optimal_thres_flg, random_state, x_norm_train_raw, y_norm_train,
                    x_norm_val_raw, y_norm_val, x_norm_test_raw, y_norm_test,
                    x_attack_1_raw, y_attack_1, factor, show_flg, balance_train_data_flg,
                    all_res_metrics_feat_list, sub_features_lst, title_flg, AE_model_file)
